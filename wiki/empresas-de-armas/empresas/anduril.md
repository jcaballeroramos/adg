---
tipo: empresa
estado: borrador
nombre: Anduril Industries
pais: Estados Unidos
fundacion: 2017
fundador: Palmer Luckey
valoracion: 14 mil millones de dólares (agosto 2024)
fuentes:
  - https://www.anduril.com/
  - https://www.axios.com/2024/08/08/anduril-14-billion-autonomous-weapons
tags: [anduril, eeuu, palmer-luckey, ia, drones, armas-autonomas, oculus]
---

# Anduril Industries

Empresa estadounidense de tecnología de defensa fundada en **2017** por **Palmer Luckey** (creador de Oculus VR antes de venderla a Facebook). Especializada en **sistemas autónomos**, IA aplicada a defensa, drones, vehículos submarinos no tripulados, sensores y software de análisis.

## Datos financieros
- **Agosto 2024**: $1.500 M de financiación, valoración total **$14.000 millones**.
- Contratos clave con la **Fuerza Aérea de EE.UU.** (programa Collaborative Combat Aircraft).
- Se destinan fondos a la **construcción de fábricas** para producción en masa de armas autónomas.

## Productos
- **Lattice OS** — sistema operativo de mando-control basado en IA.
- **Ghost** — UAV (drone) autónomo.
- **Anvil** — drone interceptor.
- **Bolt-M** — munición merodeadora.
- **Dive-LD** — vehículo submarino autónomo.
- **Sentry Tower** — torre de vigilancia autónoma (usada en frontera sur EE.UU.).

## Por qué nos importa
- Es la **cara visible** del nuevo complejo militar-tech de Silicon Valley.
- Su producto **Sentry Tower** se usa en la frontera entre EE.UU. y México.
- Encarna el siguiente paso del paradigma "no letal": **armas autónomas** que delegan la decisión a un algoritmo.
- Conecta directamente con [[../../herramientas/laws-armas-autonomas]] y con la posición de la ONU sobre LAWS.

## Conexiones
- ↔ [[vannevar-labs]] y [[shield-ai]] — el "trío" Silicon Valley en defensa.
- ↔ [[../../herramientas/laws-armas-autonomas]] — el debate sobre Lethal Autonomous Weapons Systems.
- ↔ [[../../herramientas/symposium-non-lethal-weapons-2026]] — el foro donde se normaliza este tipo de tecnología.
- ↔ [[../../autores-y-referencias/paul-rocher/paul-rocher-entrevista]] — *"el aparato represivo se rodea no solo de ingenieros sino también de escritores de ciencia ficción y futurólogos"*.

## Fuentes
- **Axios** (8 ago 2024) — *«Anduril now valued at $14 billion, set to build autonomous weapons factories»*: https://www.axios.com/2024/08/08/anduril-14-billion-autonomous-weapons — Anduril recauda **$1.5 mil millones** en ronda, alcanza **$14 mil millones** de valoración, construirá fábricas de armas autónomas. Contrato con la US Air Force para aviones de combate colaborativos.

## Pendientes
- [ ] Documentar a fondo Sentry Tower y su uso real en la frontera sur.
- [ ] Localizar reportajes críticos sobre Palmer Luckey y la cultura de la empresa.
