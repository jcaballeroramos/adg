---
tipo: empresa
estado: stub
nombre: Combined Systems Inc. (CSI)
pais: Estados Unidos
fundacion: 1981
subsidiarias: ["Combined Tactical Systems (CTS)", "Penn Arms"]
producto_emblematico: Sistema VENOM
fuentes:
  - https://www.combinedsystems.com/
tags: [combined-systems, csi, cts, penn-arms, venom, eeuu, latam, palestina, egipto]
---

# Combined Systems Inc. (CSI / CTS / Penn Arms)

Empresa **estadounidense** fundada en **1981**. Principal proveedor mundial de **municiones menos letales y sistemas de lanzamiento** para control de disturbios, equipos tácticos policiales y unidades militares. Subsidiarias: **Combined Tactical Systems (CTS)** y **Penn Arms**.

## Datos
- 🌐 https://www.combinedsystems.com/
- Fundada **1981**
- Subsidiarias clave: **CTS** y **Penn Arms**

## Productos clave
- **Sistema VENOM** — lanzador múltiple "multiplicador de fuerza"
- Granadas aturdidoras
- Gases lacrimógenos
- Granadas de impacto y de picadura (sting-ball)
- Aerosoles irritantes
- Lanzadoras múltiples

## Marketing
> "Una compañía ha estado a la altura del desafío de proporcionar soluciones menos letales: **CSI y sus subsidiarias CTS y Penn Arms**."
>
> Sobre el **VENOM**: "ahora hay una nueva solución no letal. Un **multiplicador de fuerza** con efectos escalables […] Venom puede ayudar a separar combatientes de no combatientes, escalar la fuerza, mejorar la protección de la fuerza en puestos de control de vehículos, puntos de control de entrada y operaciones de convoy, **determinar la intención**."

→ ya analizado en [[../feindef/02-cts-venom]].

## Mercado y presencia
- Contratos con el **Cuerpo de Marines y la Marina de EE.UU.** desde **2006**.
- **Mayor presencia en América Latina**, especialmente Centroamérica y México.
- Ventas del **Sistema VENOM a Colombia** (usado por el ESMAD).
- Implicada en **casos de corrupción en Bolivia** por intermediarios.
- Vende **armamento muchas veces caducado** a LATAM.
- Cuestionamientos por prácticas comerciales y venta de armamento inadecuado a las necesidades operativas.

## Casos documentados
- **[[../../casos/latam/colombia-2021|Colombia — Paro Nacional 2021]]** — Sistema VENOM usado por ESMAD, disparado horizontalmente.
- **[[../../casos/latam/bolivia-2019|Bolivia 2019]]** — armamento caducado, casos de corrupción.
- **[[../../casos/latam/ecuador-2019|Ecuador 2019]]** — vía intermediario Juan Francisco Herdoíza Palacios.
- **[[../../casos/latam/guatemala-2020|Guatemala 2020]]** — vía intermediario Tactical Group, sobreprecio.
- **[[../../casos/latam/chile-estallido-2019|Chile 2019]]**.
- **[[../../casos/latam/venezuela-2017-2019|Venezuela]]** — uso documentado.
- **México** — armas vencidas en circulación.
- **[[../../casos/latam/transferencias-cross-country|Préstamos LATAM]]** — material CTS prestado entre Colombia y Ecuador.
- **Territorios Ocupados por Israel** — usado contra palestinos.
- **Egipto 2011** — represión.

## Conexiones
- ↔ [[00-indice]]
- ↔ [[../feindef/02-cts-venom]] — el bloque FEINDEF dedicado a CTS.
- ↔ [[../../casos/latam/intermediarios-latam]] — la familia Fajardo (Imdicol Colombia), Tactical Group (Guatemala), Herdoíza Palacios (Ecuador).
- ↔ [[../../casos/latam/transferencias-cross-country]].

## Pendientes
- [ ] Documentar contratos militares estadounidenses (Marines, Navy).
- [ ] Buscar el documento donde declaran "determinar la intención" en relación al VENOM.
